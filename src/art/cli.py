...
def create_local_backend_app(backend: LocalBackend) -> FastAPI:
    """Create a FastAPI app with all routes and middleware for the given LocalBackend."""
    fastapi_app = FastAPI()
    ...
    fastapi_app.get("/healthcheck")(lambda: {"status": "ok"})
    fastapi_app.post("/close")(backend.close)
    fastapi_app.post("/register")(backend.register)
    fastapi_app.post("/_get_step")(backend._get_step)
    fastapi_app.post("/_delete_checkpoints")(backend._delete_checkpoints)
...
def run(host: str = "0.0.0.0", port: int = 7999) -> None:
    ...
    fastapi_app = create_local_backend_app(backend)
